...
int main() {
    ...
    std::ofstream outfile("ground_truth.csv");
    std::string line;
    ...
    for (int iter = 0; iter < num_iterations && std::getline(infile, line); ++iter) {
        ...
        outfile << line << "," << perfResults[0].algo << "\n";
        //std::cout << "Algorithm: " << perfResults[0].algo << ", Time: " << perfResults[0].time << "ms, Memory: " << perfResults[0].memory/1048576 << " MB" << std::endl;
