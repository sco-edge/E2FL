...
def get_num_classes(dataset_name: str):
    if dataset_name in ["cifar10", "mnist", "fashion_mnist"]:
        ...
        return 1000
    # Text / LLM datasets — not classification tasks
    text_datasets = [
        "vicgalle", "alpaca-gpt4",
        "imdb", "ag_news", "yelp_review", "sst2",
        "alpaca", "oasst", "openhermes", "llama-instructions",
    ]
    if any(dataset_name.startswith(t) for t in text_datasets):
        return None
...
def load_vision_data(dataset_name, partition_train_test):
